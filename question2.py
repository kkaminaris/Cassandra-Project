...
with open('test.csv','w') as outfile:
    ...
    rows = session.execute('SELECT * from movie_details where movie_title = \'Jumanji\' order by tag_counter desc limit 5')
    for row in rows:
        temprow = []
        temprow.append(row.movie_id)
        temprow.append(row.movie_title)
        temprow.append(row.movie_year)
        temprow.append(row.movie_genres)
        temprow.append(round(row.avg_rating, 1))
        temprow.append(row.tag_name)
        temprow.append(row.tag_counter)
